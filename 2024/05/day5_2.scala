...
def main(file: String = "input.txt") = {
  ...
  def reshuffle(order: List[Int]) =
    order.find { o =>
      val befores = order.filter(b => map.find(m => m.before == b && m.after == o).isDefined)
      val afters = order.filter(a => map.find(m => m.before == o && m.after == a).isDefined)

      befores.size == afters.size
    }.get
  ...
  order.foldLeft(0) {
    case (count, nextOrder) =>
      Range(0, nextOrder.size).toList.foldLeft(true) {
        ...
        case true => count
        case _ => count + reshuffle(nextOrder)
      }
  }
}