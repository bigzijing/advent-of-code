...
def main(file: String = "input.txt") = {
  ...
  def getSteps(coords: List[Coord], traversedPaths: Set[Coord]): Int =
    iterateMovement(coords, traversedPaths).map(c => ((c.i, c.j))).size

  val topAndBottom = Range(0, columns).toList.flatMap(j => List(Coord(0, j, Southwards), Coord(rows - 1, j, Northwards)))
  val leftAndRight = Range(0, rows).toList.flatMap(i => List(Coord(i, 0, Eastwards), Coord(i, columns - 1, Westwards)))

  val possibleStarts = topAndBottom ++ leftAndRight

  possibleStarts.foldLeft(-1) {
    case (acc, next) =>
      val newSteps = getSteps(List(next), Set.empty)

      if (newSteps > acc) newSteps
      else acc
  }
}
